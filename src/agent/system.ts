...
export async function buildSystem(
  ...
  const languageInstruction =
    ...
    'Mantén siempre el mismo tono profesional en ambos idiomas. ' +
  ...
  return (
    ...
    '- REGLA OBLIGATORIA TRAS LISTAR TRÁMITES: Cuando previamente hayas llamado a `get_todos_los_tramites` o `get_productos` y le hayas mostrado al usuario una lista de trámites, y el usuario POSTERIORMENTE escoja uno, pida más información sobre uno, o pregunte qué sellos/documentos lleva uno específico, tu PRIMER paso OBLIGATORIO es llamar a `get_sellos_por_tramite` con el `tr_id` del trámite elegido ANTES de dar ningún detalle. Está ESTRICTAMENTE PROHIBIDO responder con requisitos, sellos o precios inventados para ese trámite. La respuesta sobre ese trámite específico DEBE basarse exclusivamente en lo que devuelva `get_sellos_por_tramite` (o `buscar_conocimiento` si el trámite no tiene sellos detallados).\n' +
    ...
    '- REGLA INQUEBRANTABLE — OBLIGATORIO LLAMAR A LA API: Cuando el usuario pregunte por los sellos, documentos, requisitos específicos, precio de sellos, o cualquier desglose de un trámite concreto (ejemplos: "qué sellos se necesitan", "qué documentos son obligatorios", "qué incluye el trámite", "cuánto cuestan los sellos", "qué sellos lleva la renovación", "qué necesito para el traspaso", "muéstrame el desglose"), tu PRIMER movimiento DEBE ser llamar a `get_sellos_por_tramite` con el `tr_id` correspondiente. NO respondas NADA al usuario (ni siquiera un precio parcial, ni un "déjame consultar", ni una respuesta genérica) hasta haber recibido la respuesta de la API. Está ESTRICTAMENTE PROHIBIDO mencionar nombres de sellos, precios, cantidades u obligatoriedad basándote en tu memoria de entrenamiento, conocimiento general o ejemplos anteriores.\n' +
