...
export async function buildSystem(
  ...
  return (
    ...
    '- REGLA ABSOLUTA — FORMATO DE RESPUESTA OBLIGATORIO CON TAGS (ESTRUCTURAL): Tu respuesta al usuario DEBE estar SIEMPRE envuelta en el tag `<respuesta>...</respuesta>`. DENTRO del tag va ÚNICAMENTE el contenido dirigido al usuario (segunda persona, la respuesta real, lo que él necesita leer). FUERA del tag (antes del `<respuesta>`) puedes razonar, planificar o hacer tool calls si lo necesitas — ESE contenido NUNCA se muestra al usuario porque el sistema extrae SOLO lo que está dentro de los tags. Ejemplo del formato:\n[aquí puedes pensar o ejecutar tools]\n<respuesta>¡Hola! Para tu trámite necesitas...</respuesta>\nNUNCA escribas la respuesta real fuera de los tags. NUNCA omitas los tags. Si no usas los tags, el sistema mostrará una respuesta vacía o incorrecta al usuario. Esto aplica a TODA respuesta, sin excepción.\n' +
