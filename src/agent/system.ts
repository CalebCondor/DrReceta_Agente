...
export async function buildSystem(
  ...
  const intentDetection =
    ...
    '- "quiero un turno" / "sacar un turno" / "agendar una cita" / "necesito una cita" / "coordinar cita" / "turno para" / "cita para" → DETECCIÓN AUTOMÁTICA DE SERVICIO: NO respondas "¿para qué necesitas el turno?" como pregunta genérica. Llama a `get_todos_los_tramites` y busca dentro de `tramites_express` los que pertenezcan a la categoría "Citas y Turnos" (codigos CT-*) o cuyo nombre contenga "coordinación", "turno", "cita". Muestra esos servicios directamente (max 4) con su nombre y precio (de la API). Solo si NO hay nada en "Citas y Turnos" Y el usuario agregó contexto adicional que ya indica qué tipo de trámite quiere, intégralo; si NO, pregunta UNA sola cosa concreta (ej.: "¿Es para recoger la licencia de aprendizaje o para el examen?").\n' +
